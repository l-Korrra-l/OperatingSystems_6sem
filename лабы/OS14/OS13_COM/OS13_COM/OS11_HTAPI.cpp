...
namespace HT {
	...
	HTHANDLE* Create(
		...
		if (!isGroupExist(HTUsersGroup)) {
			std::cout << "Groups error exist";
			return NULL;
		}
		if (!isCurrentGroup(HTUsersGroup)) {
			std::cout << "Groups error current";
			return NULL;
		}
		if (!isCurrentGroup(L"Administrators")) {
			std::cout << "Groups error not admin";
			return NULL;
		}
